...
import noUiSlider,{target,Formatter} from 'nouislider';
...
class CustomNouislider {
  ...
  constructor(id: string, min: number, max: number, tooltipsIdPart: string) {
    ...
    this.min = min;
    this.max = max;
    this.formatSlider = document.getElementById(this.id) as target;
  ...
  initSlider = (): void => {
    ...
    if (this.formatSlider.noUiSlider === undefined)
      throw new Error("formatSlider.noUiSlider - undefined");

    this.set(this.min, this.max);

    const startValues = document.getElementById(
      `filter__${this.tooltipsIdPart}_start`
    );
    const endValues = document.getElementById(
      `filter__${this.tooltipsIdPart}_end`
    );
    if (!startValues || !endValues)
      throw new Error("startValues or endValues is null");
